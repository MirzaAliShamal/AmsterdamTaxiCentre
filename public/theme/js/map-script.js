...
function initMap() {
    ...
    var autocomplete = new google.maps.places.Autocomplete(spoint,{componentRestrictions:{country:["nl"]}});
    var epoint = document.getElementById('epoint');
    var autocomplete1 = new google.maps.places.Autocomplete(epoint,{componentRestrictions:{country:["nl"]}});
            ...
            for (var i = 0; i < origins.length; i++) {
                ...
                for (var j = 0; j < results.length; j++) {
                    ...
                    if (element.distance.value <= miles_limit) {
                        ...
                        var init_fare = (element.distance.value * rate) / 1000;
                        var fare = parseInt(init_fare) + parseInt(base_fare);
                        $(".distance-error .error strong").html("");
                    }
                    
                    total_minutes = element.duration.value / 60;
                    total_distance = element.distance.value / 1000;
                    ...
                    // $('#results').html('<h4>€'+Math.ceil(fare)+'.00 '+ (element.distance.value / 1000).toFixed(1) +' KM ('+duration_text+')</h4>');
                }
            }
        }
    }
